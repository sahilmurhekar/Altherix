...
const PrescriptionsPage = () => {
  ...
  const filtered = filterPrescriptions();
  ...
  const PrescriptionModal = ({ prescription, onClose }) => {
    ...
    const daysRemaining = Math.ceil((new Date(prescription.expiryDate) - new Date()) / (1000 * 60 * 60 * 24));
        ...
        <div className="bg-zinc-900 border border-zinc-700 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="sticky top-0 bg-gradient-to-r from-red-600/20 to-pink-600/20 border-b border-zinc-700 p-6 flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-400 mt-1">
                From {prescription.doctor} • {prescription.specialization}
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className={`rounded-lg p-4 border ${getStatusColor(prescription.status)}`}>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-red-500/10 to-pink-500/10 border border-red-500/30 rounded-2xl p-6 md:p-8">
        ...
        <p className="text-zinc-400">View and manage all your prescriptions</p>
      </div>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-zinc-400">{prescription.specialization}</p>
                  </div>
                  <div className={`px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColor(prescription.status)}`}>
                    {prescription.status === 'active'
                      ? `${daysRemaining} days left`
                      : 'Expired'}
                  </div>
                ...
                <div className="mb-4 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {prescription.medicines.map((medicine, idx) => (
                      <span key={idx} className="inline-block bg-zinc-800/50 px-3 py-1 rounded-full text-xs text-zinc-300">
